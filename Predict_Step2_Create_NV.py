...
'''
def process_row(row):
    #GeneName,Family,Sub_Index,Sub_Seq,Sub_Seq_N
    ...
    # Calculate NV_Masked for the Sub_Seq_N
    #NV_Masked = calculate_NV(sub_seq_n, 'NV_344')
    NV_Masked = calculate_NV(sub_seq_n, 'NV_88')
    results.append([gene_name, family, sub_index, sub_seq, sub_seq_n] + NV_Masked)
...
#file_path = 'sProtein_alpha_mask.csv' #GeneName,Family,Sub_Index,Sub_Seq,Sub_Seq_N
file_path = 'sProtein_delta_mask.csv'
...
#s_NV_file = open('sProtein_alpha_mask_NV344.csv', 'w', newline='')
s_NV_file = open('sProtein_delta_mask_NV88.csv', 'w', newline='')
s_NV_writer = csv.writer(s_NV_file)
s_NV_writer.writerow(['GeneName'] + ['Family'] + ['Sub_Index'] + ['Sub_Seq']  + ['Sub_Seq_N'] + [f'V{i+1}' for i in range(NV_dim)])
